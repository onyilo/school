<?php 
	...
	if ($id>0 and $action =="" )
	{
		$sql ="Select * from class_master where id = $id";
		$rs = mysql_query($sql);	
	...
	elseif ($action !=""){
		...
		$sql = "Select * from formmasters where arm_id='$arm_id' and class_id='$class_id'" ;
		$rs = mysql_query($sql);
		if (mysql_num_rows($rs)>0 and $action!="Delete")
		{
			$record = mysql_fetch_assoc($rs);
			$msg="<font color='#ff0000' style='font-size:14px;'>". $record["class_name"]." ".$record["arm_name"]." already has form master (".$record['surname']." ".$record['firstname']." ".$record['othername'].") assigned</font>";
			$id = 0;
			$class_id ="";
			$action ="";
			$arm_id = "";
		...
		elseif($action == "Add")
		{
			$sql = "Insert into class_master(class_id, arm_id, staff_id,c_session_id, posted_by) values('$class_id','$arm_id', '$staff_id','".getCurrentSession()."','$userID')";
			mysql_query($sql) or die(mysql_error());
			$msg="<font color='#005500' style='font-size:14px;'>Record Inserted Successfully</font>";
		}
		elseif($action == "Update")
		{
			$sql = "Update class_master Set class_id='$class_id', arm_id='$arm_id', staff_id='$staff_id',last_modify_by='$userID',last_modify_date=Current_Timestamp() where id = $id";
			mysql_query($sql);
			$id = 0;
			$class_id ="";
			$action ="";
...
<table class="mytable" style=" margin:auto;border-collapse:collapse">
	<tr>
        ...
        <th>Class</th>
        <th>Arm</th>
        <th>Form Master Name</th>
        <th></th>
    </tr>
    <?php
    	$sql ="Select * from formmasters  order by class_name, arm_name";
		...
		}else
		{
			while($row = mysql_fetch_assoc($rs))
			{
				...
				echo "<td><a href='home.php?page=form_master&id=".$row["id"]."'>Edit</a></td>";
				echo "</tr>";
