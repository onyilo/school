<?php 
	...
	if ($id>0 and $action =="" )
	{
		$sql ="Select * from classsubjectconfig where id = $id";
		$rs = mysql_query($sql);
	...
	elseif ($action !=""){
		...
		elseif($action == "Add")
		{
			$sql = "Insert into classsubjectconfig(subject_id,class_id, arm_id, offertypeID, posted_by) values('$subject_id','$class_id','$arm_id', '$OfferTypeID','$userID')";
			mysql_query($sql);
			$msg="<font color='#005500' style='font-size:14px;'>Record Inserted Successfully</font>";
		}
		elseif($action == "Update")
		{
			$sql = "Update classsubjectconfig Set subject_id='$subject_id', class_id='$class_id', arm_id='$arm_id', offertypeID='$OfferTypeID',last_modify_by='$userID',last_modify_date=Current_Timestamp() where id = $id";
			mysql_query($sql);
			$id = 0;
			$class_id ="";
			$subject_id = "";
		...
		{
			$sql = "Delete from classsubjectconfig where id= $id";
			mysql_query($sql);
			$id = 0;
			$class_id ="";
			$subject_id = "";
...
<tr>
     ...
     <tr>
	  <tr>
		...
		<td>Offer Type</td>
        <td><?php echo GenerateListBox("Select OfferTypeID, OfferType from OfferTypes", "$OfferTypeID","OfferTypeID","","Select Subject"); ?></td>
     </tr> 
	  <tr>
		<td>Class</td>
...
<table class="mytable" style=" margin:auto;border-collapse:collapse">
    ...
    <?php
		...
		while($row = mysql_fetch_assoc($rs))
		{
			...
			echo "<td><a href='home.php?page=classsubjectconfig&id=".$row["id"]."'>Edit</a></td>";
			echo "</tr>";
